"""FastAPI application – health / debug endpoints.

The primary interface is the MCP server (stdio), but this app provides
HTTP health-check and optional debug routes for manual tool invocation.
"""
...
async def lifespan(app: FastAPI):
    logger.info("Financial MCP server starting (env=%s)", settings.app_env)
    yield
...
async def debug_search_companies(query: str = Query(...), limit: int = Query(10)):
    result = await handle_search_companies({"query": query, "limit": limit})
